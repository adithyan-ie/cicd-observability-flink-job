...
/**
 * Reports the job's current event-time watermark as a GLOBAL-pipeline_id
 * MetricResult, so it can be read off the dashboard when constructing
 * "truly late" test events (see DeploymentFrequencyOperator.ALLOWED_LATENESS
 * / TRULY_LATE_TAG). Upserted whenever the watermark advances — checked both
 ...
 * backlog is fully drained) the reported value — and the "Current
 * Watermark" dashboard panel — would sit frozen at whatever it was last
 * time an element arrived, even though the watermark had actually kept
 ...
 * stream only emits when the watermark actually advances.
 */
